...
.DESCRIPTION
    ...
    If the file is present, the function returns this path. If not, the function returns nothing.
.EXAMPLE
    ...
    If neither kcat nor kafkacat were found, the function returns nothing.
.EXAMPLE
...
.EXAMPLE
    ...
    The function returns the full path to the file named "kafkacat", if the file is present in the current environment. If not, the function returns nothing.
.INPUTS
    System.String
.OUTPUTS
    System.String
...
function Get-KafkaCatPath {
    ...
    param   (
        ...
        if  ($Win32)    {   [string]$kafkaPath  =   [System.IO.Path]::ChangeExtension($kafkaPath,'.exe')    }
